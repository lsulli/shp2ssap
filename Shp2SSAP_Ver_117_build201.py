...
"""
Shp2SSAP_Ver_117_build200.py file sorgente per Shp2SSAP.exe ver 1.1.7 build 201
...
# history
# - build 201 - 2017.05.03 - MATPLOTLIIB opzionale - disattivato di default
...
mtpl_chk = 0
# MATPLOTLIB START
# import Comment/uncomment: to make smaller executable if matplotlib not running

# WARNING: advance idle as PyCharm show some error code (without executable problem) if import is comment

##if mtpl_chk == 1:
##	from matplotlib.pyplot import show as plt_show
##	from matplotlib.pyplot import figure as plt_figure
##	from matplotlib.pyplot import plot as plt_plot
##	from matplotlib.pyplot import legend as plt_leg
##	from matplotlib.pyplot import close as plt_cls
##	from matplotlib.pyplot import axis as plt_axis
##	from matplotlib.pyplot import grid as plt_gr
##	from matplotlib.pyplot import subplots_adjust as plt_sp_adj
##	from matplotlib.pyplot import text as plt_txt
...
# default label
versione = "1.1.7 - build 201"
root_title = "Da shapefile a file per SSAP - Shp2SSAP [ Ver. " + versione + "]"

# default value
xy2shp_forSSAP_str = "xy2Shp_forSSAP_093_020.py"
...
try:
	...
	def check_function(pass_check):
		...
		try:
			...
			if check_ok == 1 and pass_check == 1:
				tkinter.messagebox.showinfo(msg_title_check,
	...
	def shp_2_ssap_files():
		...
		try:
			...
			f_dat.write(chr(124) + "File .dat per SSAP2010 generato da Shp2SSAP, versione " + versione + "\n")
			...
			# MATPLOTLIB START
			# close all matpltlib figure open and kill plot
			if mtpl_chk == 1:
				plt_cls()
			# MATPLOTLIB END
			...
			while main_counter < count_feat_dat_fld:
				...
				x_ref1sx, y_ref1sx, x_ref2sx, y_ref2sx, x_ref1dx, y_ref1dx, x_ref2dx, y_ref2dx = xy_ref(sf.shapeRecord(d_mc))
				print ("id: ",sf.shapeRecord(d_mc).record[id_ssap_id], "ref coord: ",  x_ref1sx, y_ref1sx, x_ref2sx, y_ref2sx, x_ref1dx, y_ref1dx, x_ref2dx, y_ref2dx)
				...
				if ((sf.shapeRecord(d_mc).record[id_ssap].casefold()) == "dat"
					...
					# MATPLOTLIB START
					# set EMPTY list and tuple to plot
					if mtpl_chk == 1:
						list_x_plt, list_y_plt = set_empty_tuple()
						myxlbl_list = []
						myylbl_list = []
					# MATPLOTLIB END
					...
					for k in sf.shapeRecord(d_mc).shape.points:
						...
						if check_trimming.get() == 1:
							if k[0] < (x_min_topo + xlenght_tolerance):  # x a sinistra del limite topografico + tolleranza
								if k[0] == x_min:  #
									var_x = x_min_topo
							...
							elif k[0] > (x_max_topo - xlenght_tolerance):  # x a destra del limte topografico - tolleranza
					...
					# MATPLOTLIB START
						if mtpl_chk == 1:
							myxlbl_list.append(round(var_x, 2))
							myylbl_list.append(round(var_y, 2))
							list_x_plt.append(round(var_x, 2))
							list_y_plt.append(round(var_y, 2))
					if mtpl_chk == 1:
						myxlbl_list.sort()
						myylbl_list.sort()
						plt_txt(myxlbl_list[-1], myylbl_list[-1], "[" + str(sf.shapeRecord(d_mc).record[id_ssap_id]) + "]")
						dat_plt = plt_plot(tuple(list_x_plt), tuple(list_y_plt), color="sienna")
						dat_plt, = dat_plt
					# MATPLOTLIB END
					# manage counter
				...
				# ###### start write .fld file #######
					...
					# MATPLOTLIB START
						if mtpl_chk == 1:
							...
							list_y_plt.append(round(var_y, 2))
					if mtpl_chk == 1:
						fld_plt = plt_plot(tuple(list_x_plt), tuple(list_y_plt), color="blue", ls='--')
						fld_plt, = fld_plt
						list_x_plt, list_y_plt = set_empty_tuple()
					# MATPLOTLIB END
			...
			if count_feat_svr > 0 and chk_fld_svr == 1:
				...
				while svr_counter < count_feat_svr:
					...
					if ((sf.shapeRecord(d_mc_svr).record[id_ssap].casefold()) == "svr"
						...
						# MATPLOTLIB START
						if mtpl_chk == 1:
							list_x_plt.append(round(var_x_min, 2))
							list_x_plt.append(round(var_x_max, 2))
							list_y_plt.append(round(var_y_max_svr, 2))
							list_y_plt.append(round(var_y_max_svr, 2))
							plt_txt(round(var_x_min, 2), round(var_y_max_svr, 2) + 1,
									"S" + str(sf.shapeRecord(d_mc_svr).record[id_ssap_id]))
							svr_plt = plt_plot(tuple(list_x_plt), tuple(list_y_plt), color="grey", lw=5)
							svr_plt, = svr_plt
							list_x_plt, list_y_plt = set_empty_tuple()
						# MATPLOTLIB END
						svr_counter += 1
			...
			if count_feat_sin > 0:
				...
				if mtpl_chk == 1:
					list_x_plt, list_y_plt = set_empty_tuple()
				# MATPLOTLIB END
				f_sin.write("# Shp2SSAP - versione: " + versione + "\n")
				...
				while sin_counter < count_feat_sin:
					...
					if sf.shapeRecord(d_mc_sin).record[id_ssap].casefold() == "sin":
						...
						# MATPLOTLIB START
						if mtpl_chk == 1:
							sin_plt = plt_plot(tuple(list_x_plt), tuple(list_y_plt), color="red", ls='--')
							sin_plt, = sin_plt
							list_x_plt, list_y_plt = set_empty_tuple()
						# MATPLOTLIB END
					else:
			...
			if checkerr_run == 1:
				...
				if mtpl_chk == 1:
					...
					myleg_lb = []
					try:
						myleg.append(dat_plt, )
						myleg_lb.append("dat")
					except:
						pass
					try:
						myleg.append(fld_plt, )
						myleg_lb.append("fld")
					except:
						pass
					try:
						myleg.append(svr_plt, )
						myleg_lb.append("svr")
					except:
						pass
					try:
						myleg.append(sin_plt, )
						myleg_lb.append("sin")
					except:
						pass
					...
					# show and set plot and figure
					plt_leg(myleg, myleg_lb, loc=4)
					plt_sp_adj(bottom=0.10, right=0.95, left=0.05, top=0.9)
					plt_gr(True)
					plt_axis('equal')
					fig = plt_figure(1)
					fig.canvas.set_window_title(f_mod.name)
					plt_show()
			# MATPLORLIB END
			else:
		...
		# and name of function that catch error
			...
			if checkerr_run == 0 and checkerr_pre == 0:
				...
				tkinter.messagebox.showerror(msg_title_error, "Si e' verificato un errore non previsto!\n"
											+ "La procedura termina \n\n Descrizione dell'errore: \n"
											+ str(sys.exc_info()[0]) + "\n"
	...
	if mtpl_chk == 1:
		def set_empty_tuple():
			"""
			Crea tuple vuote per matplotlib
			"""
			try:
				my_x_list = []
				my_y_list = []
