...
Nel 1° campo deve essere inserita una sequenza continua ed ordinata numerica intera univoca per ogni tipologia SSAP
indicata nel successivo campo (2° campo)
...
Versione 1.1.6 built 24 - 2015.04.03
...
#check SSAP limit number of layers
		if check_layers_number(sf,num_shape, "dat",20)== True:
			msg_err_tot = msg_err_tot + msg_err_layer_num
			checkerr1 = 1
		if check_layers_number(sf,num_shape, "svr",10)== True:
			msg_err_tot = msg_err_tot + msg_err_layer_num
			checkerr1 = 1
		if check_layers_number(sf,num_shape, "fld",1)== True:
			msg_err_tot = msg_err_tot + msg_err_layer_num
			checkerr1 = 1
#check if ID is continuous for "dat" record
		if count_fields_dat_fld > 0:
			if check_continuous_order(sf,num_shape,"dat")==True:
				msg_err_tot = msg_err_tot + msg_err_continuous_order
				checkerr1 = 1
		else:
			msg_err_tot = msg_err_tot + "nel secondo campo utente non è presente nessun layer di tipo 'dat'. \t\n\n"
			checkerr1 = 1
...
def check_layers_number (my_shape,layer_num, type_ssap, max_num):
	...
	try:
		...
		if type_ssap_num > max_num:
			...
			+" è pari a " +str(type_ssap_num)+ ", superiore a " +str(max_num)+", limite ammesso da SSAP.\t\n\n")
			return True
		else:
			return False
...
def check_continuous_order (my_shape,layer_num, type_ssap):
	...
	try:
		...
		global msg_err_continuous_order_n2
		msg_err_continuous_order = ""
		msg_err_continuous_order_n1 = ""
		msg_err_continuous_order_n2 = ""
		f=0
...
#get id value
			...
			if my_shape.shapeRecord(f).record[1] != "fld":
				if my_shape.shapeRecord(f).record[0]  == 0:
						msg_err_continuous_order_n2 = ("- è presente uno strato SSAP = '" + str (my_shape.shapeRecord(f).record[1]) + "' con ID = 0 che non è di tipo falda. ID = 0 è ammesso solo per gli strati SSAP 'fld'")
			f+=1
		...
		if (id_list_ordered != perfectorder):
			...
			return True
		elif len (msg_err_continuous_order_n2) > 0:
			msg_err_continuous_order = msg_err_continuous_order_n1+"\n"+ msg_err_continuous_order_n2+"\n"
			return True
		else:
			return False
