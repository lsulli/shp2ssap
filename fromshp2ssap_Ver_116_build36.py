...
- Verificare Errore scrittura campi C e CU quando si vogliono condizioni miste drenate/non drenate
...
try:
	...
	#main loop: read shapefile and get geometry layers and points for .dat and .fld file, get attribute for .geo file
			while mc < count_feat_dat_fld:
				...
				print ("chk_fld_rock ="+str(chk_fld_rock))
	...
	#write data layer in .geo file
					if chk_fld_rock==0 or (chk_fld_rock==1 and (sf.shapeRecord(a).record[id_sigci])==0) :
						if CheckCu.get()==1:
							print ('scelta c/cu:'+str(CheckCu.get()))
					...
					elif (chk_fld_rock==1 and (sf.shapeRecord(a).record[id_sigci])>0):
						f_geo.write("\t"+str(0))
						f_geo.write("\t"+str(0))
						f_geo.write("\t"+str(0))
						f_geo.write("\t"+str(sf.shapeRecord(a).record[id_gamma]))
