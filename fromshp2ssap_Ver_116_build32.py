...
TEST FILE - NON FUNZIONANTE - aggiornati i riferimenti agli indici
...
Richiede il modulo shapefile.py (giÃ  presente di default nella distribuzione di fromshp2ssap.py)
...
In particolare é richiesta una struttura dati del tipo seguente, 
...
Nel campo SSAP deve essere indicato a quale file ssap é riferita la polyline.
Valori ammessi: .dat, .fld e .svr. Il file .geo é generato in base ai valori dei campi PHI, C, CU etc.
Nel campo VAL1 é indicato il valore in kpa dei carichi
...
Il file .mod potrÃ  essere aperto direttamente da SSAP 
...
Versione 1.1.6 built 32 - 2015.04.28
Autore: Lorenzo Sulli - lorenzo.sulli@gmail.com
L'uso della procedura fromshp2ssap.py é di esclusiva responsabilitÃ  dell'utente, 
In accordo con la licenza l'autore non é responsabile per eventuali risultati errati o effetti dannosi 
su hardware o software dell'utente
Si ringrazia l'autore del modulo shapefile.py, alla base di tutte le funzionalitÃ  del presente script.
...
try:
	...
	#last exit for error input, ordinary way use active_disable_button()
			if os.path.exists(shape_path)==False:
				checkerr1=1
				msg_err_tot = "Non é stato selezionato lo shapefile di Input o lo shapefile indicato non esiste \t \n"
				sys.exit()
	#set main shape variables
			try:
				sf = shapefile.Reader(shape_path)
			except:
				tkinter.messagebox.showerror(msg_title_error, "Si é verificato un errore nel caricamento dello shapefile \n"+"La procedura termina \n\n Descrizione dell'errore: \n" 
	...
	#check if ID is continuous for "dat" record
			if count_feat_dat_fld > 0:
				...
				msg_err_tot = msg_err_tot + "Nel campo SSAP non é presente nessun layer di tipo 'dat'. \t\n\n"
				checkerr1 = 1
	...
	#last exit for error input, ordinary way use active_disable_button()
			if os.path.isdir(os.path.dirname(ssap_pathname)) == False:
				checkerr1=1
				msg_err_tot = "Non é stato indicato il nome dei file SSAP di output \t \n"
	...
	#main loop: read shapefile and get geometry layers and points for .dat and .fld file, get attribute for .geo file
			while mc < count_feat_dat_fld:
				print ("num_shape del ciclo é = ", count_feat_dat_fld, " - mc del ciclo é = ", mc)
	...
	#write data layer in .geo file
					if (sf.shapeRecord(a).record[id_sigci])==0:
						if CheckCu.get()==1:
	...
	#loop: read shapefile and get geometry layers and points for .svr
				while svr_counter < count_feat_svr:
					print ("num_shape del ciclo per svr é = ", count_feat_svr, " - svr_counter del ciclo é = ", svr_counter)
	#write polyline only when id field = mc counter
	...
	#specific error are handled by type default_msg_unknow_error function that return system error and name of function that catch error
			...
			if checkerr1==0:
				tkinter.messagebox.showerror(msg_title_error, "Si é verificato un errore!\n"+"La procedura termina \n\n Descrizione dell'errore: \n" 
				+str(sys.exc_info()[0])+ "\n" + str(sys.exc_info()[1])+msg_err_function)
			elif checkerr1==1:
	...
	def check_layers_number (my_shape,layer_num, type_ssap, max_num):
		...
		try:
			...
			if type_ssap_num > max_num:
				...
				+" é pari a " +str(type_ssap_num)+ ", superiore a " +str(max_num)+", limite ammesso da SSAP.\t\n\n")
				return True
			else:
				return False
	...
	def check_points_number (points_num, layer_id):
		...
		try:
			if (points_num)> 100:
				...
				+ " é superiore a 100, limite ammesso da SSAP.\t\n")
				return True
			else:
				return False
		except:
	...
	def check_topdown_order (mylist_min,mylist_max,mylist_centroid):
		...
		verifica della funzione check_continuous_order che si basa sui valori degli indici e non é in grado
		...
		try:
			...
			if (mylist_min != myorderlist_min) and (mylist_max != myorderlist_max) and (mylist_centroid != myorderlist_centroid):
				...
				+ "probabili cause:\n"+"- la seguenza verticale é continua ma non é ordinata\t\n"
				+"- é presente una lente o un muro che non rispetta la numerazione secondo gli standard SSAP\n\n"
	...
	def check_continuous_order (my_shape,layer_num, type_ssap):
		...
		non é possibile discriminare eventuali incongruenze geometriche.
		Funzione richiamata dalla funzione principale shp2ssapfile()
		"""
		try:
	...
	#get id value
			...
			if (id_list_ordered != perfectorder):
				...
				+"\n\n" + "Per i layer '" + type_ssap + "' La seguenza degli indici ordinata é: " + str(id_list_ordered)
				+ "\n\n possibili cause:\n"+"- la seguenza dell'indice é continua ma piÃ¹ strati hanno lo stesso ID \t\n"
				+"- la seguenza dell'indice non é continua\n" + msg_err_continuous_order_n1+"\n")
				return True
			else:
				return False
		except:
	...
	def check_shape_type (my_shapes):
		...
		try:
			...
			if my_shapes[0].shapeType != 3: # 3 = polyline type in ESRI specification
				msg_err_shape_type = ("Lo shapefile non é del tipo 'polyline' \n\n")
				return True
			else:
				return False
		except:
	...
	def check_flds_ssap(my_sf):
		...
		try:
			...
			for a in my_flds:
				...
				fld_set.add(b)
				if a[0] == "USER_ID":
					id_user_id = my_flds.index(a)-1 #remove ID completion Field
			...
			if fld_ssap_set_datgeo.issubset(fld_set)==False:
				return "Per creare file dat e geo sono necessari i seguenti campi: " + str_lack_builder(lack_ssap_fld_datgeo)
			if fld_ssap_set_svr.issubset(fld_set)==False:
				return "Per creare file dat, geo e svr mancano i seguenti campi:" + str_lack_builder(lack_ssap_fld_svr)
			if fld_ssap_set_rock.issubset(fld_set)==False:
				return "Per creare file dat, geo per strati con rocce  mancano i seguenti campi: " + str_lack_builder(lack_ssap_fld_rock)
			if fld_ssap_set_other.issubset(fld_set)==False:
				return "Per creare file dat, geo e altro mancano i seguenti campi: " + str_lack_builder(lack_ssap_fld_other)
			if fld_ssap_set_tot.issubset(fld_set)==False:
				return "Complessivamente mancano i seguenti campi: " + str_lack_builder(lack_ssap_fld_tot)
				if len(ok_ssap_fld_tot)>0:
					return "Sono giÃ  presenti i seguenti campi per SSAP: " + str_lack_builder(ok_ssap_fld_tot)
			else:
				return "Campi presenti ed indici assegnati!"
		except:
		...
		# try:
			...
			# if num_flds < 12:
				# msg_err_shape_fields_number = ("Lo shapefile ha n." + str(len(my_flds)-1) + " campi definiti dall'utente. \n"
												# + "I campi devono essere almeno 12.\n"
												# +msg_flds_structure+"\n\n")
				# return True
			# else:
				# return False
	...
	# def check_shape_fields_type (my_flds):
		...
		# try:
			...
			# for a in my_flds[1:2]:
				# xcount +=1
				# if a[1]!='N':
					# msg_err_tmp = ("Il 1Â° campo utente '" + str(a[0]) + "' dello shapefile non é numerico.")
					# msg_err_shape_fields_type = msg_err_shape_fields_type + msg_err_tmp
			# for b in my_flds[2:3]:
				# xcount +=1
				# if b[1]!='C':
					# msg_err_tmp = ("Il 2Â° campo utente '" + str(b[0]) + "' dello shapefile non é stringa.")
					# msg_err_shape_fields_type = msg_err_shape_fields_type + msg_err_tmp
			# for c in my_flds[3:13]:
				# xcount +=1
				# if c[1]!='N':
					# msg_err_tmp = ("Il " +str(xcount)+ "Â° campo utente '" + str(c[0]) + "' dello shapefile non é numerico.")
					# msg_err_shape_fields_type = msg_err_shape_fields_type + msg_err_tmp
	...
	def check_cu_value (cu_value, layer_id):
		try:
			...
			if cu_value == 0:
				...
				+ " é zero, la verifica in condizioni non drenate non é applicabile.\t\n")
				return True
			else:
				return False
		except:
