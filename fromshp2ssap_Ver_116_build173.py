#!/usr/bin/env pythonw
# Versione 1.1.6 build 173 alfa - 2016.10.25
# Bottone per xy2shpforssap
# TEST FILE - In sviluppo FUNZIONAmento parziale
# tentativo di verificare se xy2shp_forssap è già in esecuzione - Non Risolto
...
Il file di Input, completo della struttura tabella come descritta sotto, 
può essere creato con apposito tool da un elenco coordinate x-y 
disposte su colonne separate da uno dei seguenti caratteri: 
carattere tabulazione, carattere punto e virgola (;)/carattere barra verticale (|).
La struttura dati richiesta è la seguente, è obbligatorio rispettare nome e tipologia dei campi, 
...
Obbligatorio (Peso di volume naturale - KN/mc): ['GAMMA', 'N', 5, 2]
...
Opzionale (Valore di pressione per il file .svr - Kpa):['VAl1','N', 5, 2]

Nel campo SSAP_ID deve essere inserita una sequenza continua ed ordinata numerica intera univoca con base 1
per ogni tipologia SSAP .dat e .svr (indicata nel campo SSAP), il tipo falda (SSAP = 'fld')
e il tipo superficie singola (SSAP = 'sin') devono essere unici con SSAP_ID = 0

	IMPORTANTE: il sistema controlla la sequenza verticale continua degli strati .dat e .svr. 
	il controllo può essere escluso nel caso di modelli dle pendio molto complesssi 
	per cui la verifica della sequenza può dare falsi errori.

Nel campo SSAP deve essere indicato a quale file ssap e' riferita la polyline.
Valori ammessi: .dat, .fld, .svr. e .sin.
Il file .geo e' generato in base ai valori dei campi PHI, C, CU etc.

Nel campo VAL1 e' indicato il valore di pressione (in kpa) dei carichiescritto dal file .svr.
...
Il campo EXCLUDE è impostato di default con 0 o null, valori diversi da 1 indicano che il layer deve essere convertito.
	...
	IMPORTANTE: Se si escludono layer 'dat' o alcuni layer 'svr' deve essere ridefinita una seguenza continua dall'alto al basso
	dei valori di SSAP_ID.

Il campo DR_UNDR permette di impostare la verifica drenata/non drenata per ogni singolo strato .dat
consentendo di registrare negli attributi contemporaneamente i valori C, CU e PHI,
il file .geo corrispondete rispetterà le specifiche SSAP riportando o il valore di Cu o i valori di PHI, C.

Sono implementate funzioni di contollo della struttura degli shapefile di input.

A procedura conclusa positivamente saranno creati sempre i file SSAP 
.dat, .geo,  e .mod. 
i file .fld, .svr e .sin saranno presenti se indicati nel campo SSAP.
...
L'uso della procedura shp2ssap.py e' di esclusiva responsabilità dell'utente, 
...
msg_fld_str = ""
root_title = "Da shapefile a file per SSAP - Shp2SSAP.exe [Ver. "
...
try:
	...
	def check_function(pass_check):
		...
		try:
			...
			print ("numero di oggetti  ", num_shape)
...
def open_form():
	try:
		if os.path.exists ('xy2Shp_forSSAP.exe'):
			proc = subprocess.Popen('xy2Shp_forSSAP.exe', creationflags=subprocess.SW_HIDE, shell=False)
		else:
...
try:
		...
		if os.path.exists('Shp2SSAP.ico'):
